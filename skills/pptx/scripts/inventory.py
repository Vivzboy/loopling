...
# .potx compatibility — must come after pptx imports
try:
    from _pptx_compat import open_presentation
except ImportError:
    open_presentation = lambda p: Presentation(str(p))  # noqa: E731
...
The output JSON includes:
  ...
  - Issue detection: text overflow and shape overlaps
    ...
    if input_path.suffix.lower() not in (".pptx", ".potx"):
        print("Error: Input must be a PowerPoint file (.pptx or .potx)")
        sys.exit(1)
